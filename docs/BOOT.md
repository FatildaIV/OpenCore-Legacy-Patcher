...
Reboot the machine while holding `Option` to select the EFI Boot entry with the OpenCore icon (holding the `Option (⌥)` or `Alt` key will make this the default boot entry):
